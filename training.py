...
def main(_run, _config, _log):
    ...
    print(f'###### Using augmentation: {_config["which_aug"]} ######')
    ...
    log_loss = {'loss': 0, 'align_loss': 0, 'otsu_loss':0}
    ...
    for sub_epoch in range(n_sub_epoches):
        ...
        for _, sample_batched in enumerate(trainloader):
            ...
            ### otsu
            loss = query_loss + align_loss + otsu_loss
            ### otsu end
            loss.backward()
            optimizer.step()
            scheduler.step()
            ...
            # otsu_loss = otsu_loss.detach().data.cpu().numpy() if otsu_loss != 0 else 0

            _run.log_scalar('loss', query_loss)
            _run.log_scalar('align_loss', align_loss)
            # _run.log_scalar('otsu_loss', otsu_loss)
            log_loss['loss'] += query_loss
            log_loss['align_loss'] += align_loss
            # log_loss['otsu_loss'] += otsu_loss
            ...
            if (i_iter + 1) % _config['print_interval'] == 0:
                ...
                # otsu_loss = log_loss['otsu_loss'] / _config['print_interval']

                log_loss['loss'] = 0
                log_loss['align_loss'] = 0
                # log_loss['otsu_loss'] = 0

                print(f'step {i_iter+1}: loss: {loss}, align_loss: {align_loss}')#, otsu_loss: {otsu_loss}')
            ...
            #     return 1 # finish up
        if sub_epoch==29:
            _log.info('###### save the final model ######')
