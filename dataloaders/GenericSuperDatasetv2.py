...
class SuperpixelDataset(BaseDataset):
    def __init__(self, which_dataset, base_dir, idx_split, mode, transforms, scan_per_load, num_rep = 2, min_fg = '', nsup = 1, fix_length = None, tile_z_dim = 3, exclude_list = [], superpix_scale = 'SMALL', **kwargs):
        ...
        print(f'###### Number of scans: {len(self.img_pids)} ######')
        print(f'###### {self.img_pids} ######')
        print(f'###### type of scans: {type(self.img_pids)} ######')
        # exit()
        ...
        self.scan_ids = self.get_fold(idx_split, mode, json_file_path = 'dataloaders/splits_final_040.json')
        print("use 040.json to get the scan_ids")
...
## newly add: read classmap_1.json -->> read classmap_100.json
    ...
    def supcls_pick_binarize(self, super_map, sup_max_cls, bi_val = None):
        ...
        if bi_val == None:
            if sup_max_cls<=1:
                # print(f'###### supcls_pick_binarize sup_max_cls: {sup_max_cls} ######')
                # exit()
                bi_val = 1
            else:
    ...
    def __getitem__(self, index):
        ...
        # print(f'###### sup_max_cls: {sup_max_cls} ######')
        # exit()
        if sup_max_cls < 1:
