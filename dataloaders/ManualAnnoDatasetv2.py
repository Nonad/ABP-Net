...
def pad_img(img):
    ...
    w = (ww - 256) // 2
    if h > 0 and w > 0:
        img = img[:, h:256 + h, w:256 + w]
    elif h > 0:
        img = np.pad(img[:, h:256 + h, :],((0, 0), (0, 0), (-w, 256 + w - ww)),
                                        mode='constant', constant_values=0)
    elif w > 0:
        img = np.pad(img[:, :, w:256 + w],
                                        ((0, 0), (-h, 256 + h - hh), (0, 0)),
                                        mode='constant', constant_values=0)
    else:
        img = np.pad(img,((0, 0), (-h, 256 + h - hh),(-w, 256 + w - ww)),
                                        mode='constant', constant_values=0)
    return img
...
class ManualAnnoDataset(BaseDataset):
    def __init__(self, which_dataset, base_dir, idx_split, mode, transforms, scan_per_load, min_fg = '', fix_length = None, tile_z_dim = 3, nsup = 1, exclude_list = [], extern_normalize_func = None,**kwargs):
        ...
        self.scan_ids = self.get_fold(idx_split, mode, json_file_path = 'dataloaders/splits_final_040_test.json')
    ...
    def get_scanids(self, mode, idx_split):
        ...
        print(f"self.nsup:{self.nsup}")
        print(f"potential_support_sid:{self.potential_support_sid}")
        # exit()
        if mode == 'train':
    ...
    def get_fold(self, idx_split, mode, json_file_path):
        ## idx_split: 0-4
        def get_val_id(data, index):
            if index < 0 or index > 4:
                raise ValueError("Index must be between 0 and 4.")
            return data[index].get(str(index), [])
        with open(json_file_path, 'r') as file:
            data = json.load(file)
            # read josn data
            val_ids = get_val_id(data, idx_split)
            # if idx_split == 4:
            #     val_ids = val_ids.insert(0, '002')
            # print(val_ids)
        if mode == 'train':
            return [ ii for ii in self.img_pids if ii not in val_ids ]
        elif mode == 'val':
    ...
    def read_dataset(self):
        ...
        ## self.img_lb_fids : dict contains all the image and label file
        for scan_id, itm in self.img_lb_fids.items():
            ...
            for ii in range(1, img.shape[-1] - 1):
                # print(f"img.shape[-1] - 1:{img.shape[-1] - 1}")
                ...
                # print(f'scan_id:{scan_id},ii:{ii}, glb_idx:{glb_idx}')
                glb_idx += 1
...
## newly add: read classmap_1.json -->> read classmap_100.json
    ...
    def get_support(self, curr_class: int, class_idx: list, scan_idx: list, npart: int):
        ...
        print(f"get_support.pid_curr_load:{self.pid_curr_load}")
        # ['049', '050', '051', '052', '053', '054', '002', '005', '006']
        print(f"get_support.scan_idx:{scan_idx}")
        # get_support.scan_idx:[4]
        print(f"get_support.potential_support_sid:{self.potential_support_sid}")
        # get_support.potential_support_sid:['053']
        print(f'###### Using {len(scan_idx)} shot evaluation!')

        ## npart=3
        # pcts:[0.1667, 0.5, 0.8333]
        if npart == 1:
            pcts = [0.5]
        else:
            half_part = 1 / (npart * 2)
        ...
        for _part in range(npart):
            ...
            for scan_order in scan_idx:
                ...
                _zlist = self.tp1_cls_map[self.label_name[curr_class]][_scan_id] ## get the list of slices containing the current class
